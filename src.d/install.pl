...
if(geteuid() != 0)
  {
    print("This installation program must be run with special privileges, i.e., as root.\n");
    exit($EXIT_FAILURE);
  }
...
print("****************************************************************************\n");
print("WARNING: Interrupting this process at any step will not undo previous steps!\n");
print("****************************************************************************\n");
...
do
  {
    printf("Which directory contains Mad Dog Backup System's source? [%s] ", $rundir);
    chop($val = <STDIN>);
...
do
  {
    ...
    if(!getpwnam($val))
      {
	printf("\"%s\" does not appear to be a valid user on this system.\n", $val);
      }
  }
while(!getpwnam($val));
...
do
  {
    ...
    if(!getgrnam($val))
      {
	printf("\"%s\" does not appear to be a valid group on this system.\n", $val);
      }
  }
while(!getgrnam($val));
...
foreach $dir (@dirs)
  {
    ...
    elsif((getpwnam($usernam))[2] != $info->uid || (getgrnam($groupnam))[2] != $info->gid)
      {
	$val = "N";

	do
	  {
	    printf("Change %s's owner and group to %s and %s, respectively? [N/y] ", $dir,
		   $usernam, $groupnam);
	    chop($val = <STDIN>);
	...
	if($val eq "y")
	  {
	    if(!chown((getpwnam($usernam))[2], (getgrnam($groupnam))[2], $dir))
	      {
...
do
  {
    printf("What is the full cgi URL of your Mad Dog Backup System? [%s] ", $cgiurl);
    chop($val = <STDIN>);
...
do
  {
    printf("What is the full home URL of your Mad Dog Backup System? [%s] ", $homeurl);
    chop($val = <STDIN>);
...
if(!chown((getpwnam($usernam))[2], (getgrnam($groupnam))[2], $backupdir . "/data"))
  {
    printf("Unable to chown() %s.\n", $backupdir . "/data");
  }
else
	...
	if($str =~ /\}\;/)
	...
	else
	  {
		...
		if(!($info = stat($userdir)))
		  ...
		  {
		    if(!chown((getpwnam($usernam))[2], (getgrnam($groupnam))[2], $userdir))
		      {
			printf("Unable to chown() %s.\n", $userdir);
		      }
		    else
